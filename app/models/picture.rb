class Picture < Cms::Base
  ...
  #######################SPECIĀLĀS FUNKCIJAS ###################################
  def tagi=(values)
    self.tags=Cms::Tag.collect_tags(values)
  end
  ...
  def add_watermarks
    ...
    if File.exist?(RAILS_ROOT+"/public/images/watermark-54.png") && self.picture
      ...
      Picture.full_versions.each{|version,dimensions|
  ...
  def self.rebuild 
    ...
    m_versions=[]
    time=Time.now
    puts "Start Picture rebuild! #{time.strftime("%Y.%m.%d %H:%M:%S")}"
  ...
  def self.new_from_params params={}
    ...
    if parent && params[:tempid]!="true"
      picture.pictureable_id=params[:parent_id]
      picture.pictureable_type=params[:parent].camelize
    else
      picture.pictureable_type=params[:parent].camelize
    end
    picture.picture=params[:picture][:picture] if params[:picture]
    picture.main_image=params[:single]=="true" ? true : nil
    if picture.picture
      picture.save!
      picture
    else
  ...
  def next_picture
    picture=Picture.by_parent(self.pictureable_type,self.pictureable_id).positioned("asc").after(self.position).first
    picture=Picture.by_parent(self.pictureable_type,self.pictureable_id).positioned("asc").first unless picture
    picture
  end

  def prev_picture
    picture=Picture.by_parent(self.pictureable_type,self.pictureable_id).positioned("desc").before(self.position).first
    picture=Picture.by_parent(self.pictureable_type,self.pictureable_id).positioned("asc").last unless picture
    picture
  end
  ...
  def singularize_main
    if @changed_attributes && @changed_attributes.has_key?('main_image') && self.main_image
      old_main=Picture.by_parent(self.pictureable_type,self.pictureable_id).main.find(:first)
  ...
  def assign_position
    if (self.pictureable_id.to_i>0 && !self.position)
      current_position=Picture.by_parent(self.pictureable_type,self.pictureable_id).maximum("position")
