class Media::VideoFile < Media::FileBase
  set_table_name :media_video_files
  ...
  def create_picture_if_needed
    if !self.picture && self.name
      ...
      destination=Media::ImageFile.get_temp_destination()
      file_name="second_1_#{self.name.to_s.gsub(/\.\w+$/,"")}.png"
      begin
        Media::ImageFile.transaction do
          ...
          picture=Media::ImageFile.create_with_parent!(self,:name=>up_image,:watermark_free=>true)
  ...
  def set_ratio
    ...
    begin
      if !self.picture && self.name
        ...
        img=::Magick::Image::read("#{RAILS_ROOT}/public/#{self.picture.name.url}").first
      end
      if img
        w=img.columns
        h=img.rows
