...
#    %(new Ajax.Request(this.value+"/allowed_actions",
#      {asynchronous:true,evalScript:true,
#       method:'POST',
#       parameters:"menu_item_id=#{mi}",
#       onComplete:function(request){
#        el=elementById('#{actions}');
...
# Pre-definition of ActionView::Helpers::FormBuilder class
# Now ruby parser can understand number_field tag
#class ActionView::Helpers::FormBuilder
#self.field_helpers += ['number_field']
##self.field_helpers += ['date_field']
#def number_field(method, options = {})
#@template.send(:number_field, @config[:object_name], method, options.merge(:object => @object))
#end
## def date_field(method, options={})
##@template.send(:date_field, @config[:object_name], method, options.merge(:object => @object))
## end
#end