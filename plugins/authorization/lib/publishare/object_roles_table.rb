        ...
        def has_role( role_name, authorizable_obj = nil )
          ...
          if role.nil?
            if authorizable_obj.is_a? Class
              role = Admin::Role.create( :name => role_name, :authorizable_type => authorizable_obj.to_s )
            elsif authorizable_obj
              role = Admin::Role.create( :name => role_name, :authorizable => authorizable_obj )
            else
              role = Admin::Role.create( :name => role_name )
            end
          end
    ...
    module ModelExtensions
      ...
      module ClassMethods
        def acts_as_authorizable
          has_many :accepted_roles, :as => :authorizable, :class_name => 'Admin::Role'
