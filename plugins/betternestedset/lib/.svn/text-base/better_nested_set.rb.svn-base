        ...
        private
          ...
          def update #:nodoc:
            connection.update(
              ...
              "SET #{quoted_comma_pair_list(connection, special_attributes_with_quotes)} " +
          ...
          # exclude the lft/rgt columns from update statements
           def special_attributes_with_quotes(include_primary_key = true) #:nodoc:
            column_names=Admin::MenuItem.column_names.collect{|name|
              ["lft","rgt"].include?(name) ? nil : name
            }.compact
      
            attributes.inject({}) do |quoted, (name, value)|
              if column = column_for_attribute(name)
                quoted[name] = quote_value(value, column) if column_names.include?(column.name)#(!include_primary_key && column.primary) || [acts_as_nested_set_options[:left_column], acts_as_nested_set_options[:right_column]].include?(column.name)
              end
              quoted
            end
          end
