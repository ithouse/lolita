module Lolita
  module Controllers
    ...
    module AuthorizationHelpers
      ...
      def current_ability
        if self.respond_to?(:is_lolita_resource?) && self.is_lolita_resource? && authentication_proxy.current_user
          if defined?(::CanCan)
            @current_ability||= ::Ability.new(authentication_proxy.current_user)
          else
            super
          end
        else
